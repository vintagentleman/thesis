...
\begin{frame}{Цель и задачи работы}
    ...
    \begin{block}<2->{Задачи}
        \begin{enumerate}
            \item<2-> изучение проблем глагольного словоизменения в церковнославянском языке XV--XVII~вв.\ в рамках задачи лемматизации
            \item<3-> программная реализация компонента для лемматизации глаголов, представленных на привлечённом материале
            \item<4-> разработка программного компонента для прецедентной разметки текстов СКАТ, грамматической разметкой не обладающих
        \end{enumerate}
    \end{block}
\end{frame}
...
\begin{frame}{Термины}
    \begin{columns}
        \column{0.5\textwidth}
        \begin{block}<1->{Нормализация}
            идентификация графического инварианта
        \end{block}
        \column{0.5\textwidth}
        \begin{block}<2->{Лемматизация}
...
\begin{frame}{Лемматизация}
    \begin{columns}
        \column{0.5\linewidth}
        \begin{block}<1->{Алгоритм}
            \begin{enumerate}
                \item<2-> нормализация
                \item<3-> отсечение флексии и формативов
                \item<4-> \alert<6->{преобразование основы}
                \item<5-> добавление словарной финали
            \end{enumerate}
        \end{block}

        \column{0.5\linewidth}
        \begin{block}<1->{{\agio ѡбрѣтохомъ}}
            \begin{itemize}
                \item[]<2-> {\agio обрѣтохомъ}
                \item[]<3-> {\agio обрѣт}
                \item[]<4-> {\agio обрѣс}
                \item[]<5-> {\agio обрѣсти}
            \end{itemize}
...
\begin{frame}{Формы церковнославянского глагола по типу ФОС}
    \begin{columns}
        \column{0.5\linewidth}
        \begin{block}<1->{От основы настоящего времени}
            \begin{enumerate}
        ...
        \column{0.5\linewidth}
        \begin{block}<2->{От основы инфинитива}
            \begin{enumerate}
                \item аорист
                \item имперфект
                \item причастия прошедшего времени
                \item эловые причастия
                \item \alert<3->{инфинитив}
                \item \textcolor{gray}{супин}
            \end{enumerate}
        \end{block}
    \end{columns}
\end{frame}

\begin{frame}{Сопоставление классификаций глаголов}
    \centering
    \scriptsize
...
\begin{frame}{Оценка результатов}
    \begin{exampleblock}<1->{Показатели}
        \begin{itemize}
            \item полнота 100\%
...
\begin{frame}{Опыт по внедрению}
    \begin{itemize}
        ...
        \item однако качество работ не уменьшилось \begin{itemize}
